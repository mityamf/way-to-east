...
var loadForm = function loadForm() {
  ...
  function validate() {
    if (checkInput(nameField, nameField.value.length >= MIN_LENGTH && REG_NAME.test(String(nameField.value))) && checkInput(emailField, emailField.value.length >= MIN_LENGTH && REG_EMAIL.test(String(emailField.value).toLowerCase())) && checkInput(phoneField, phoneField.value.replace(/\D+/g, '').length < MAX_TEL_LENGTH) && checkInput(messageField, messageField.value.length >= MIN_LENGTH && REG_MESSAGE.test(String(messageField.value))) && checkInput(agreement, agreement.checked)) {
      return true;
    } else {
      return false;
    }
  ...
  function iterateFields(evt) {
    if (evt.target.value.length !== 0) {
      switch (evt.target.id) {
        ...
        case 'phone-field':
          checkInput(phoneField, checkInput(phoneField, phoneField.value.replace(/\D+/g, '').length < MAX_TEL_LENGTH));
          break;
