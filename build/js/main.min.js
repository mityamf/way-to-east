...
var loadForm = function loadForm() {
  ...
  function validate() {
    if (checkInput(nameField, nameField.value.length >= MIN_LENGTH && REG_NAME.test(String(nameField.value))) && checkInput(emailField, emailField.value.length >= MIN_LENGTH && REG_EMAIL.test(String(emailField.value).toLowerCase())) && checkInput(phoneField, phoneField.value.replace(/\D+/g, '').length >= MIN_TEL_LENGTH) && checkInput(messageField, messageField.value.length >= MIN_LENGTH && REG_MESSAGE.test(String(messageField.value))) && checkInput(agreement, agreement.checked)) {
      return true;
    } else {
      return false;
    }
  ...
  function iterateFields(evt) {
    if (evt.target.value.length !== 0) {
      switch (evt.target.id) {
        ...
        case 'phone-field':
          checkInput(phoneField, checkInput(phoneField, phoneField.value.replace(/\D+/g, '').length >= MIN_TEL_LENGTH));
          break;
  ...
  var setTelMask = function setTelMask() {
    if (inputsTel.length > 0) {
      inputsTel.forEach(function (input) {
        ...
        var chooseMask = function chooseMask(maskPattern) {
          var setMask = function setMask() {
            ...
            mask = new imask__WEBPACK_IMPORTED_MODULE_0__["default"](input, maskOptions);
          };
